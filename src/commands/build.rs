...
async fn is_command_available(command: &str) -> bool {
    ...
    Command::new(if cfg!(target_os = "windows") { "cmd" } else { "sh" })
...
pub async fn execute(
    ...
    if !klave_config_path.exists() {
        return Err(anyhow!("Error: klave.json file not found. Make sure you are in a Klave project directory. Run 'klave create' to start a new project."));
    }
    
    // Read the klave config
    let klave_config_str = fs::read_to_string(&klave_config_path)
        .context("Failed to read klave.json")?;
    
    let klave_config: Value = serde_json::from_str(&klave_config_str)
        .context("Invalid JSON in klave.json")?;
    
    // Get applications array
    let applications = klave_config.get("applications")
        .and_then(|v| v.as_array())
        .ok_or_else(|| anyhow!("Error: Invalid klave.json file structure. 'applications' array not found."))?;
    ...
    println!("Building {}", if let Some(app_name) = &app {
    ...
    for application in apps_to_process {
        let app_slug = application.get("slug")
            .and_then(|s| s.as_str())
        ...
        if !app_dir.exists() {
            eprintln!("{}", format!("Warning: Directory not found for app \"{}\" at {:?}", app_slug, app_dir).yellow());
        ...
        if app_type == "unknown" {
            eprintln!("{}", format!("Warning: Could not determine app type for \"{}\"", app_slug).yellow());
            build_results.push(BuildResult {
                app: app_slug.to_string(),
                success: false,
        ...
        let build_result = match app_type {
            "rust" => {
                ...
                if !has_cargo {
                    Err(anyhow!("Rust toolchain not found. Please install Rust from https://rustup.rs/"))
                } else if !has_cargo_component {
                    Err(anyhow!("cargo-component not found. Please install with: cargo install cargo-component"))
                } else {
                    // Build Rust application
                    run_command(
                        "cargo",
                        &["component", "build", "--target", "wasm32-unknown-unknown", "--release"],
                        &app_dir,
                        true,
                    ).await.map(|_| ())
                }
            ...
            "assemblyscript" => {
                ...
                if !has_node {
                    Err(anyhow!("Node.js not found. Please install Node.js from https://nodejs.org/"))
                } else if !has_npm {
                    Err(anyhow!("npm not found. It usually comes with Node.js installation."))
                } else {
            ...
            Err(error) => {
                ...
                if app_type == "rust" {
                    if !has_cargo {
                        ...
                        println!("    - Add WebAssembly target: rustup target add wasm32-unknown-unknown");
                        ...
                        println!("    - Make sure you also have the WebAssembly target: rustup target add wasm32-unknown-unknown");
